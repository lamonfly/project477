...
void init()
{
	...
	camera.sideView(_matrix, _matrixI);
...
void handleKeyPress(unsigned char key, int x, int y)
{
	...
	{
	case 's':
		base[index].setDamp(base[index].getDampen() - 0.01f);
		cout << "Dampen :" << base[index].getDampen() << endl;
		break;
	case 'd':
		base[index].setDamp(base[index].getDampen() + 0.01f);
		cout << "Dampen :" << base[index].getDampen() << endl;
		break;
	case 'w':
		base[index].setTimeStep(base[index].getTimeStep() - 0.001f);
		cout << "Time Step :" << base[index].getTimeStep() << endl;
		break;
	case 'e':
		base[index].setTimeStep(base[index].getTimeStep() + 0.001f);
		cout << "Time Step :" << base[index].getTimeStep() << endl;
	...
	case 'r':
		...
		anim_playing = false;
		init();
		base[index].setTimeStep(0.1f);
		base[index].setPlay(false);
		base[index].setDamp(1.0f);
		cout << "RESET" << endl;
		break;
	case 'z':
		if (index > 0) {
	...
	case 'f':
		...
		break;
	case 'o':
		camera.topView(_matrix, _matrixI);
		break;
	}
}
